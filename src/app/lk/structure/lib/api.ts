  ...
  balance: {
    ...
    getRubleBalance: async () => {
      try {
        ...
        if (!token) {
          console.error('API: Токен авторизации не найден при получении рублевого баланса');
          throw new Error('Необходима авторизация. Пожалуйста, войдите в систему и попробуйте снова.');
        }
        
        console.log('API: Получение рублевого баланса...');
        ...
        console.error('Ошибка при получении рублевого баланса:', error);
    ...
    getBonusBalance: async () => {
      try {
        ...
        if (!token) {
          console.error('API: Токен авторизации не найден при получении бонусного баланса');
          throw new Error('Необходима авторизация. Пожалуйста, войдите в систему и попробуйте снова.');
        }
        
        console.log('API: Получение бонусного баланса...');
        ...
        console.error('Ошибка при получении бонусного баланса:', error);
        // Возвращаем значение по умолчанию вместо ошибки
        return {
          balance: 780
        };
